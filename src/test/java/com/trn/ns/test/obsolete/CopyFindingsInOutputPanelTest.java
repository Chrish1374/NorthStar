...
public class CopyFindingsInOutputPanelTest extends TestBase {
	...
	public void test02_US1281_TC6640_verifyCopyFindingsForSR() throws InterruptedException{
		...
		verifyDeSelectedSelectAllIcon(5, 15);
		checkThumbnailWhenNotSelected(6, 15,0,panel.thumbnailList.size());
	...
	@Test(groups = { "Chrome", "IE11", "Edge", "US1281","DE1846", "Positive"})
//	public void test03_US1281_TC6642_DE1846_TC7456_verifyCopyFindingsUsingkeyboardAndMouse() throws InterruptedException{
//
//		extentTest = ExtentManager.getTestInstance();
//		extentTest.setDescription("Verify the selection of the findings in the output panel using keyboard and mouse actions.");
//
//		// Loading the patient on viewer
//		patientListPage = new PatientListPage(driver);
...
//
//		ExtentManager.customExtentReportLog(LogStatus.INFO, extentTest, "", "Creating multiple points for different status");
...
//		panel.enableFiltersInOutputPanel(true, true, true);		
//
//		verifyDeSelectedSelectAllIcon(1, 12);
//		checkThumbnailWhenNotSelected(2, 12,0,panel.thumbnailList.size());
//
//		ExtentManager.customExtentReportLog(LogStatus.INFO, extentTest, "", "Selecting the specific findings");
//		panel.pressControlLeft(panel.findingsList.get(0));
//
//		verifyDeSelectedSelectAllIcon(3, 12);
//		checkThumbnailWhenSelected(4, 12,0, 1);
//		checkThumbnailWhenNotSelected(5, 12,1, panel.thumbnailList.size());
//
//		ExtentManager.customExtentReportLog(LogStatus.INFO, extentTest, "", "Verifying on clicking cntrl + left finding is deselected");
//		panel.pressControlLeft(panel.findingsList.get(0));
//		panel.mouseHover(panel.studySummary.get(0));
//
//		verifyDeSelectedSelectAllIcon(6, 12);
//		checkThumbnailWhenNotSelected(7, 12,0,panel.thumbnailList.size());
//
//		ExtentManager.customExtentReportLog(LogStatus.INFO, extentTest, "", "Selecting the even rows and checking the even findings");
//		for(int i=0;i<panel.thumbnailList.size();i++) {
//
//			if(i%2==0)
//				panel.pressControlLeft(panel.findingsList.get(i));
//
//		}
//		for(int i=0;i<panel.thumbnailList.size();i++)
//			if(i%2==0)
//				checkThumbnailWhenNotSelected(8, 12, i+1, i+2);
////				panel.assertEquals(panel.getCssValue(panel.getThumbnailRow(i+1),NSGenericConstants.BACKGROUND_COLOR),ViewerPageConstants.BACKGROUND_COLOR_THUMBNAIL,"Checkpoint[8/12]","");
//
//		ExtentManager.customExtentReportLog(LogStatus.INFO, extentTest, "", "Delecting the even rows and checking the even findings");
//		for(int i=0;i<panel.thumbnailList.size();i++) {
//
//			if(i%2==0)
//				panel.pressControlLeft(panel.findingsList.get(i));
...
//		for(int i=0;i<panel.thumbnailList.size();i++)
//			if(i%2==0)
//				checkThumbnailWhenSelected(checkpoint, finalCheckPoint, startIndex, endIndex);
//				panel.assertEquals(panel.getCssValue(panel.getThumbnailRow(i+1),NSGenericConstants.BACKGROUND_COLOR),ViewerPageConstants.BLACK_COLOR_1,"Checkpoint[9/12]","");
//
//	    panel.openAndCloseOutputPanel(false);
//		ExtentManager.customExtentReportLog(LogStatus.INFO, extentTest, "", "Selecting the findings by holding shift key pressed and clicking on left click");
//	    panel.click(panel.getViewPort(2));
//	    panel.enableFiltersInOutputPanel(true, true, true);	
//	
//		panel.holdShiftKeyPressed();
//		for(int i=0;i<3;i++) 
//		panel.click(panel.findingsList.get(i));
//		//panel.scrollDown(panel.findingsList.get(i));
//		panel.releaseShiftKeyPressed();
//
//		panel.scrollIntoView(panel.findingsList.get(0));
//		panel.mouseHover(panel.studySummary.get(0));
//		checkThumbnailWhenSelected(10, 12, 0, 3);
//		checkThumbnailWhenNotSelected(11, 12,3,panel.thumbnailList.size());
//		
//		//for(int i=0;i<2;i++)
//		panel.pressControlLeft(panel.findingsList.get(0));		
//		
...
//		for(int i=0;i<panel.thumbnailList.size();i++) {
//			if(!((i==1) || (i==2)))
//				panel.assertEquals(panel.getCssValue(panel.getThumbnailRow(i+1),NSGenericConstants.BACKGROUND_COLOR),ViewerPageConstants.BLACK_COLOR_1,"Checkpoint[12/12]","");
//			else
//				panel.assertEquals(panel.getCssValue(panel.getThumbnailRow(i+1),NSGenericConstants.BACKGROUND_COLOR),ViewerPageConstants.BACKGROUND_COLOR_THUMBNAIL,"Checkpoint[12/12]","");
//
//		}
//
//		panel.openAndCloseOutputPanel(false);
